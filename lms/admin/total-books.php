    ...
    <section class="container-fluid py-3">
        ...
        <div class="col-lg-10">
            <div class="pagination">
                <pre class="d-flex">
                    <p>Admin</p> / <a href="dashboard.php">Dashboard</a> / <a href="total-books.php">Total Books</a>
                </pre>
            </div>
            <div class="book-table">
                <div class="selection mt-2 mb-4">
                    <div class="select">
                        <p>Select Type</p>
                    ...
                    <?php
                        if(mysqli_num_rows($books_data) > 0) {
                            ...
                            while($book_result = mysqli_fetch_assoc($books_data)) {
                                // Assuming the image path is relative to the current script or absolute
                                // If images are in a specific admin folder, adjust: $imagePath = '../admin/'.$book_result['image'];
                                $imagePath = htmlspecialchars($book_result['image']); // Use htmlspecialchars for image path too
                                echo "
                                    <tr>
                                        <td>".$counter++."</td>
                                        <td>".htmlspecialchars($book_result['title'])."</td>
                                        <td><img src='".$imagePath."' alt='Book Image' width='100px' height='150px'></td>
                                        ...
                                        <td class='text-nowrap'>";
                                            if(empty($book_result['updation_date']) || $book_result['updation_date'] == '0000-00-00 00:00:00')
                                            {
                                                echo "-";
                                            }
                                            else
                                            {
                                                echo htmlspecialchars($book_result['updation_date']);
                                            }
                                            echo "
                                        ...
                                        <td class='text-nowrap'>
                                            <a href='edit-book.php?id=".htmlspecialchars($book_result['sr_no'])."' class='btn btn-primary'>Edit</a>
                                            <a href='delete-book.php?id=".htmlspecialchars($book_result['sr_no'])."' class='btn btn-danger' onclick=\"return confirm('Are you sure you want to delete this book?');\">Delete</a>
                                        </td>
                                    </tr>
                                ";
                            }
                ...
                <div class="pagination-controls">
                    <?php
                    $pagination_base_url = "total-books.php?";
                    if (!empty($selected_book_type)) {
